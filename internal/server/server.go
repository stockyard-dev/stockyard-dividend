package server
import("encoding/json";"net/http";"github.com/stockyard-dev/stockyard-dividend/internal/store")
...
func(s *Server)routes(){
    s.mux.HandleFunc("GET /health",func(w http.ResponseWriter,r *http.Request){writeJSON(w,200,map[string]string{"status":"ok","service":"stockyard-dividend"})})
    s.mux.HandleFunc("GET /api/stats",s.handleOverview)
    s.mux.HandleFunc("GET /api/referrals",s.handleList)
    s.mux.HandleFunc("POST /api/referrals",s.handleCreate)
    s.mux.HandleFunc("POST /api/referrals/{id}/signup",s.handleSignup)
    s.mux.HandleFunc("POST /api/referrals/{id}/pay",s.handlePay)
    s.mux.HandleFunc("DELETE /api/referrals/{id}",s.handleDelete)
