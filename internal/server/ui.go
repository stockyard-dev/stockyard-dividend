...
async function load(){var r=await fetch(A+'/affiliates').then(function(r){return r.json()});items=r.affiliates||[];document.getElementById('stats').innerHTML='<div class="st"><div class="st-v">'+items.length+'</div><div class="st-l">Total</div></div><div class="st"><div class="st-v">-</div><div class="st-l">&nbsp;</div></div><div class="st"><div class="st-v">-</div><div class="st-l">&nbsp;</div></div>';render();}
function render(){var q=(document.getElementById('search').value||'').toLowerCase();var f=items;
if(q)f=f.filter(function(i){return (i.name||'').toLowerCase().includes(q)||(i.email||'').toLowerCase().includes(q)||(i.code||'').toLowerCase().includes(q)});
if(!f.length){document.getElementById('list').innerHTML='<div class="empty">No affiliates.</div>';return;}
...
if(i.code)h+='<span>'+esc(String(i.code))+'</span>';
if(i.commission_pct)h+='<span>'+esc(String(i.commission_pct))+'</span>';
if(i.total_earned)h+='<span>'+esc(String(i.total_earned))+'</span>';
if(i.total_paid)h+='<span>'+esc(String(i.total_paid))+'</span>';
h+='<span>'+ft(i.created_at)+'</span></div>';
h+='</div>';});
document.getElementById('list').innerHTML=h;}
async function del(id){if(!confirm('Delete?'))return;await fetch(A+'/affiliates/'+id,{method:'DELETE'});load();}
function formHTML(item){var i=item||{name:'',email:'',code:'',commission_pct:'',total_earned:'',total_paid:'',referrals:'',status:''};var isEdit=!!item;
var h='<h2>'+(isEdit?'EDIT':'NEW')+' AFFILIATE</h2>';
...
h+='<div class="fr"><label>Code</label><input id="f-code" value="'+esc(i.code||'')+'" ></div>';
h+='<div class="fr"><label>Commission Pct</label><input id="f-commission_pct" value="'+esc(i.commission_pct||'')+'" ></div>';
h+='<div class="fr"><label>Total Earned</label><input id="f-total_earned" value="'+esc(i.total_earned||'')+'" ></div>';
h+='<div class="fr"><label>Total Paid</label><input id="f-total_paid" value="'+esc(i.total_paid||'')+'" ></div>';
h+='<div class="fr"><label>Referrals</label><input id="f-referrals" value="'+esc(i.referrals||'')+'" ></div>';
...
async function submit(){var body={name:document.getElementById('f-name').value.trim(),email:document.getElementById('f-email').value.trim(),code:document.getElementById('f-code').value.trim(),commission_pct:document.getElementById('f-commission_pct').value.trim(),total_earned:document.getElementById('f-total_earned').value.trim(),total_paid:document.getElementById('f-total_paid').value.trim(),referrals:document.getElementById('f-referrals').value.trim(),status:document.getElementById('f-status').value.trim()};
if(editId){await fetch(A+'/affiliates/'+editId,{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)});}
else{await fetch(A+'/affiliates',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)});}closeModal();load();}
